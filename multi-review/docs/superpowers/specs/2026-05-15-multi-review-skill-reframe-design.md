...
> **Archival — implemented and superseded.** Historical record of the work as planned. Line references point at the pre-split `multi_review.py` and may not match current code. The anticipated `claude -p` billing change is deferred indefinitely; current behaviour lives in `CLAUDE.md`, `README.md` and `skills/multi-review/SKILL.md`.

**Date:** 2026-05-15
**Status:** historical design implemented through v0.2 and superseded by later headless-driver work.
**Driver (historical):** The anticipated Anthropic billing change was the original motivation for the Claude Code skill split. That change is now deferred indefinitely.
