...
- [ ] **Step 3: Rewrite SKILL.md, agents, and delete dead docs**
  ...
  - Step 1: drop the `--resume-pair`/`--report` extract items and the whole "Resolve central path… CENTRAL_PATH" paragraph. In the `--list-reviewers` branch, make the host-backend wording version-neutral: change "Task subagent for claude in v0.2" to "Task subagent for claude" and drop the obsolete "Replaces v0.1's flag…" history aside.
  ...
  - Step 5: replace `--mode-override <pass1_mode>` with the literal `--mode-override reference`. This is a task-boundary bridge: Task 1's surviving skill is already reference-only, while the current `prepare` CLI still exposes the mode axis and supports this override until Task 3 removes it. Delete the snapshot-create block; delete the "Persist the prompt location for resume" block (mkdir + `prompt-source.txt`/`.sha256` + its explanatory paragraph); delete the `<EFFORT_FLAG>` construction and its use in the spawn invocation, keeping only `<MODEL_FLAG>` + `<TASK_FLAG>`. Keep the spawn dispatch loop and the claude Task dispatch + `write_task_result` call verbatim.
  - Join barrier: unchanged.
  - Step 6 (Synthesis): unchanged.
  - Step 7 (Aggregate): collapse "Output path branches by mode" to one unconditional `<cwd>/REVIEW-<slug>.md` path; replace `--mode <pass1_mode>` with the literal `--mode reference` (the current aggregate CLI still requires it until Task 3) and drop `--pair-id <pair_id_or_omit>`. Keep the `## Summary` heading classifier behavior, but rewrite "rendered and harvested as an effective failure" to "rendered as an effective failure" because Step 8 no longer exists. Append the one-line "(Steps 3, 8–12 removed in v0.3.0.)" note here.
...
- Modify: `skills/multi-review/SKILL.md` (remove Task 1's temporary literal `--mode-override reference` / `--mode reference` bridge immediately before deleting those CLI flags)
...
- [ ] **Step 3: Implement**
  ...
  `skills/multi-review/SKILL.md`: first remove Task 1's temporary `--mode-override reference` from Step 5 and `--mode reference` from Step 7. The existing `test_skill_flags_exist` contract then stays aligned when the corresponding CLI flags are deleted below; at every completed task boundary the documented invocation is runnable.
...
- [ ] **Step 3: Implement**
  ...
  `multi_review/cli/setup.py`: remove step 1 (central-dir creation), step 4 (config.json write), step 5 (`--write-allowlist` arg + allowlist print/write). Leave `_copy_tree` unchanged; adding a general exclusion mechanism for one deleted checkout artifact is unnecessary, and it would not apply to `--dev` symlinks anyway. Surviving `main()` does: resolve `home`, install skill (copy/symlink), install agents (copy/symlink + reviewer-template render), print a JSON summary with `skill`/`agents` paths only.

  This checkout currently has a stray `./skills/multi-review/config.json` (gitignored, untracked, dev leftover from a pre-0.3.0 `mr-setup` run against this repo). `rm -f ./skills/multi-review/config.json` — tolerate it already being absent when this plan is executed. This is explicitly the checkout-relative file, not the protected user-data path `~/.claude/skills/multi-review/config.json` named in Global Constraints. Removing setup's config writer above closes the actual `--dev` recurrence path (the old writer followed the installed symlink back into the checkout); removing this existing file closes the one-time residue.
