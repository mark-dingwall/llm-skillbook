...
def test_harvest_row_appends(tmp_path):
    ...
    log = tmp_path / "runs.jsonl"
    r = subprocess.run(
        ["uv", "run", "python", "-m", "multi_review.cli.harvest_row",
         "--row-file", str(row_in), "--log", str(log)],
        capture_output=True, text=True,
    )
    assert r.returncode == 0, r.stderr
    assert log.exists()
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser()
    p.add_argument("--row-file", type=Path, required=True)
...
git commit -m "feat(cli): mr-harvest-row"
```
