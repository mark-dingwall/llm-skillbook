...
- **No test may write into the live checkout.** Setup no longer writes `config.json`, but tests that invoke `setup --dev` must still stage their own `skills/`+`agents/` copy under `tmp_path` so a future checkout-local side effect cannot mutate the real tree. A session-scoped autouse fixture in `tests/conftest.py` snapshots the obsolete config path, restores it, and fails the run on any mutation — ordering-independent by design; don't demote it to a per-test assertion.
