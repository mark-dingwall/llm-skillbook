...
def run(prior, item, manifests=[], adjudication=None): return apply_bound("apply_ledger_decisions",{"prior_rows":[prior],"decisions":[item],"manifests":[{"id":value,"finding_id":"F1"} for value in manifests],"target_seal":"seal-1","adjudication":adjudication})
class LedgerTests(unittest.TestCase):
 ...
 def test_first_adjudication_failure_retries_then_second_bounces(self):
  ...
  second=run(row(),decision("REFUTED",["proof"]),[],{"attempt":2,"status":"FAILED","decided_ids":[],"proof_artifact_id":None})
