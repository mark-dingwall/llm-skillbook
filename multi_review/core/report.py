...
def render_experiments_markdown(*, log_path: Path, reports_dir: Path) -> str:
    ...
    # ---- Notes / sidecars dirs ----
    is_archive_layout = (
        log_path.parent.name == "data"
        and log_path.parent.parent.name == "archive"
    )
    notes_dir = log_path.parent / "notes"
    # Archived logs live at runs/archive/data/runs.jsonl while their notes
    # live at runs/archive/notes/. Keep regeneration useful after a local
    # archive cleanup without changing the normal runs/runs.jsonl layout.
    archive_notes_dir = log_path.parent.parent / "notes"
    if is_archive_layout and not notes_dir.exists() and archive_notes_dir.exists():
        notes_dir = archive_notes_dir
    notes_label = (
        "runs/archive/notes/"
        if notes_dir == archive_notes_dir and is_archive_layout
        else "runs/notes/"
    )
    legacy_notes_dir = notes_dir / "legacy"

    parts: list[str] = []
    parts.append("# Deprecated inline-vs-reference comparison log\n")
    parts.append(
        "_This is deprecated historical material. Generated by `uv run python -m "
        "multi_review.cli.report regen`; do not edit by hand. This local, "
        "ignored data can name private codebases and is not current operating "
        "guidance._\n"
    )
    ...
    parts.append(
        ...
        "tie → reference-first. This legacy field does not imply a quality preference.\n"
    )

    parts.append("## Deprecation\n")
    parts.append(
        "Do not start new inline-vs-reference experiments. The accumulated "
        "evidence found no meaningful difference for sufficiently capable "
        "frontier models. The interactive skill still writes these rows for "
        "compatibility; the headless driver does not. `--no-harvest` is not a "
        "supported command, and `harvest: false` is not yet an effective opt-out.\n"
    )
    ...
    if not found_any:
        parts.append(
            f"_No sidecar narrative files found in `{notes_label}`. "
            f"Drop `{notes_label}<project>-<YYYY-MM-DD>.md` files to add per-run "
    ...
    if reports_dir.exists():
        ...
        if report_files:
            ...
            for f in report_files:
                try:
                    rel = f.relative_to(log_path.parent).as_posix()
                except ValueError:
                    rel = (
                        f"../reports/{f.name}"
                        if is_archive_layout
                        else f"reports/{f.name}"
                    )
                parts.append(f"- [{f.stem}]({rel})")
            parts.append("")

    parts.append("## Historical notes\n")
    parts.append(
        "- Preserve these rows only for historical context while the deprecated "
        "comparison subsystem is retired.\n"
    )
